...
sourceFile_dic = {'fileName': PathHelp.get_file_path(None, f'TNewBrand.xlsx.xlsx'),
                  'sourceSheet': 'company',
                  'colIndex': 1,
                  'startIndex': 0,
                  'endIndex': 1313}
...
def goto_detail(company_index, company_name, profile_id):
    ...
    try:
        full_name = new_tab_driver.find_element(By.CSS_SELECTOR, 'company-name').text
        inn = new_tab_driver.find_element(By.ID, 'clip_inn')
        ...
        #finalcial
        revenue = new_tab_driver.find_element(By.XPATH, '//*[@id="ab-test-wrp"]/div[2]/div[2]/div[1]/div/div[1]/div[1]/div[2]').text
...
def send_companyName_to_filter(company_name):
...
def main():
    ...
    for (company_index, company_name) in enumerate(all_companys):
        if company_name.__contains__('?'):
            continue
        elif company_index in range(sourceFile_dic['startIndex'], sourceFile_dic['endIndex']):
