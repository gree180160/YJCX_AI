...
def open_url(isWeek):
    ...
    for (index, ppn) in enumerate(ppn_list):
        if index in range(120, TaskManager.Taskmanger.end_index): #114 unfinished
...
def change_screenShotName(fold_path):
    pn_file = PathHelp.get_file_path(TaskManager.Taskmanger().task_name, 'Task.xlsx')
    ppn_list = ExcelHelp.read_col_content(file_name=pn_file, sheet_name='ppn', col_index=1)[TaskManager.Taskmanger.start_index: 120]
