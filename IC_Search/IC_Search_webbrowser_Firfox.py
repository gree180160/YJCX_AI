...
import webbrowser
from Manager import TaskManager, AccManage, URLManager
...
def open_url(isWeek):
    firefox_path = AccManage.firefox_path
    webbrowser.register('firefox', None, webbrowser.BackgroundBrowser(firefox_path))

    pn_file = PathHelp.get_file_path(None, f'{TaskManager.Task_IC_hot_F_manger.task_name}.xlsx')
    ...
    for (index, ppn) in enumerate(ppn_list):
        if index in range(TaskManager.Task_IC_hot_F_manger.start_index, TaskManager.Task_IC_hot_F_manger.end_index):
            url = URLManager.IC_hot_url(ppn, isWeek)
