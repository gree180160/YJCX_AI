...
class IC_Stock_Info:
    ...
    #     return result
    def description_str(self):
        result = f'{self.model or "--"}, {self.Manufacturer or "--"} , {self.supplier or "--"} , {"ICCP" if self.isICCP else "notICCP"} ,  {"SSCP" if self.isSSCP else "notSSCP"}, {"SpotRanking" if self.isSpotRanking else "notSpotRanking"} , {"HotSell" if self.isHotSell else "notHotSell"} , {self.stock_num} , {time.strftime("%Y-%m-%d", time.localtime())}'
        return result
    ...
    def descritpion_arr(self):
        result = [self.model or "--",
                  ...
                  self.supplier or "--",
                  "ICCP" if self.isICCP else "notICCP",
                  "SSCP" if self.isSSCP else "notSSCP",
                  "SpotRanking" if self.isSpotRanking else "notSpotRanking",
                  "HotSell" if self.isHotSell else "notHotSell",
                  self.stock_num,
                  self.search_date]
        return result
