...
# ru [['MW6S004NT1', '1', 'TNXP_RF']]，save to mysql
...
def anly_webdriver(cate_index, cate_name, isRU):
    try:
        count_des_list = driver.find_elements(By.CSS_SELECTOR, 'div.ant-tabs-tab-btn')
        if count_des_list.__len__() >= 4:
            count_des = count_des_list[3].text
            count_des = count_des.replace('进出口数据(', '')
            count_des = count_des.replace(')', '')
            MySqlHelp_recommanded.DBRecommandChip().wheat_record_write([[cate_name, count_des, task_name]], isRU)
        else:
            MySqlHelp_recommanded.DBRecommandChip().wheat_record_write([[cate_name, "0", task_name]], isRU)
    except Exception as e:
        LogHelper.write_log(logFile, f'cannot get count {e}')


def search(all_cates, isRu):
    if not isRu:
        set_filter_all()
    else:
        set_filter_ru()
    ...
    for (cate_index, cate_name) in enumerate(all_cates):
        ...
        if cate_name is None or cate_name.__contains__('?'):
            ...
            WaitHelp.waitfor(True, False)
            anly_webdriver(cate_index=cate_index, cate_name=cate_name, isRU=isRu)
...
def main():
    ...
    if isRu:
        search(all_cates, True)
    else:
        search(all_cates, False)
